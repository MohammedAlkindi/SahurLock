...
import { SessionConfig, SessionPreset } from '@/lib/types';
...
const PRESETS: SessionPreset[] = [
  {
    ...
    config: { durationMinutes: 25, breakDurationSec: 300, breakLimit: 1, offscreenThresholdSec: 6 }
  },
  {
    id: 'deep',
    name: 'Deep Work',
...
export function SessionConfigCard({ value, onChange, onStart, disabled }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5 shadow-xl">
      ...
      <p className="mt-0.5 text-xs text-zinc-600">Webcam frames are processed locally and never transmitted.</p>
